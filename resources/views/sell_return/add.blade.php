					...
					<table class="table bg-gray" id="sell_return_table">
			          	<thead>
				            <tr class="bg-green">
				              	...
				              	<th>@lang('lang_v1.return_quantity')(Only PCS)</th>
				        ...
				        <tbody>
				            ...
				            <tr>
				              	...
				              	@php
									if ($sell_line->sells_unit  == 'cr') {
											$unit_price_inc_tax = $sell_line->unit_price_inc_tax *  $sell_line->child_value;
										}
									else{
										$unit_price_inc_tax =	$sell_line->unit_price_inc_tax;
									}
								@endphp
				              	<td><span class="display_currency" data-currency_symbol="true">{{ $unit_price_inc_tax }}</span></td>
				              	<td><input type="text"  class=" form-control input-sm previous_sells_quantity" readonly value="{{ $sell_line->formatted_qty }} {{ $sell_line->sells_unit }}"></td>
				              	...
				              	<td>
				              		@php
										...
										if ($sell_line->sells_unit  == 'cr') {
											$qty = $sell_line->formatted_qty *  $sell_line->child_value;
										}
						            ...
						            data-msg-abs_digit="@lang('lang_v1.decimal_value_not_allowed')"
			              			data-rule-max-value="{{$qty}}"
			              			data-msg-max-value="@lang('validation.custom-messages.quantity_not_available', ['qty' => $qty, 'unit' => $sell_line->product->unit->short_name ])" 
						            >
								  ...
								  <td>
									<input type="hidden" name="products[{{$loop->index}}][child_value]" class="child-value" value="{{ $sell_line->child_value }}">
									<input type="hidden"  name="products[{{$loop->index}}][sell_unit]" class="product_sells_unit" value="{{ $sell_line->sells_unit }}">
									<input type="hidden"  class="sells_product_quantity" value="{{ $sell_line->product_quantity }}">
									<input type="hidden"  class="total_discount_product" value="{{ $sell_line->discount_product_quantity }}">
									<input type="hidden"  class="previous_bonus" value="{{ $sell_line->bonus_formated }}">
									<input type="text" name="products[{{$loop->index}}][bonus_return_quantity]" class="form-control input-sm input_number return_qty discount_product_quantity" readonly value="{{ old('bonus_return_quantity', $sell_line->bonus_formated)  }}">
								</td>
				              	<td>
...
<script type="text/javascript">
	...
	$(document).on('change', 'input.return_qty, .return_type,  #discount_amount, #discount_type', function(){
		
		//bonus count for return
		var tr = $(this).parents('tr');
		var unit = tr.find('input.product_sells_unit').val();
		var child_value = __read_number(tr.find('input.child-value'));
		var previous_sells_quantity = __read_number(tr.find('input.previous_sells_quantity'));
		var qty = __read_number(tr.find('input.return_qty'));

		if (unit === 'cr') {
			previous_sells_quantity = previous_sells_quantity*child_value;
		}
		var current_sells_product_after_return = previous_sells_quantity - qty
		var bonus = __read_number(tr.find('input.total_discount_product'));
		var sell_product = __read_number(tr.find('input.sells_product_quantity'));
		var previous_bonus = __read_number(tr.find('input.previous_bonus'));
		var total_bonus = Math.floor((current_sells_product_after_return/sell_product) * bonus) ;	
		__write_number(tr.find('input.discount_product_quantity'), total_bonus);
		update_sell_return_total(unit, child_value)
	});


	function update_sell_return_total(unit, child_value){
		var net_return = 0;
		...
		// if(unit === 'cr'){
			// 	unit_price = unit_price/child_value;
			// }
