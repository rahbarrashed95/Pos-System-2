<tr class="product_row" data-row_index="{{$row_count}}">
	<td>
		...
		@php
			...
			$max_qty_rule = ($product->sells_unit =='cr')? (int)$product->total_quantity : $product->qty_available;
			$qty =($product->sells_unit =='pc') ? (int)$product->qty_available : (int)$product->total_quantity ;
			$max_qty_msg = __('validation.custom-messages.quantity_not_available', ['qty'=> $qty, 'unit' => $product->sells_unit  ]);
		@endphp
	...
	<td>
		...
		data-rule-required="true" data-msg-required="@lang('validation.custom-messages.this_field_is_required')" @if($product->enable_stock) data-rule-max-value="{{$max_qty_rule}}" data-qty_available="{{$qty}}" data-msg-max-value="{{$max_qty_msg}}" data-msg_max_default="@lang('validation.custom-messages.quantity_not_available', ['qty'=> $qty, 'unit' => $product->unit  ])" @endif >
		...
		{{$product->sells_unit}} <p style="margin-left: 80px">Pack Size: {{ $product->unit }}</p>
		
	</td>

	<td>
	 <input type="text" name="products[{{$row_count}}][unit_price_inc_tax]" 
				class="form-control pos_unit_price_inc_tax pos_unit_price input_number" 
				value="{{ $product->sell_price_inc_tax }}" 
				 readonly />
	</td>
	<td>
		<input type="text" class="form-control input_number default_unit_price" 
				value="{{ $product->sell_price_inc_tax_in_pc }}">
	</td>
	...
	{{-- <td class="text-center v-center">
		...
		<input class="display_currency pos_line_total_texta @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif" data-currency_symbol="true" value="{{$sis_price}}">
	</td> --}}
	<td class="text-center v-center">
		@php
			$subtotal_type = !empty($pos_settings['is_pos_subtotal_editable']) ? 'text' : 'hidden';
      		$sis_price=($product->quantity_ordered*$product->default_subtotal);
		@endphp
		<input type="{{$subtotal_type}}" class="form-control pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{@num_format($product->quantity_ordered*$product->sell_price_inc_tax)}}">
	...
	<td>
		@php
			$product_quantity = ($quantity !=null) ? $quantity->product_quantity : 0;
			$discount_product_quantity = ($quantity !=null) ? $quantity->discount_product_quantity : 0;
			$bonus = ($product->bonus != null ) ? $product->bonus : (($unit == 'cr' && $quantity !=null ) ? floor(($product->child_value / $product_quantity)*  $discount_product_quantity ) : 0);	
		@endphp
		<input type="hidden" class="child-value" value="{{ $product->child_value }}">
		<input type="hidden" class="product-unit" value="{{ $unit }}">
		<input type="hidden"  class="sells_product_quantity" value="{{ $product_quantity }}">
		<input type="hidden"  class="total_discount_product" value="{{ $discount_product_quantity }}">
		<input type="text" name="products[{{$row_count}}][discount_product_quantity]" class="discount_product_quantity form-control" value="{{ old('discount_product_quantity', $bonus) }}">
	</td>
	<td class="text-center">
		
		<input type="hidden" name="products[{{$row_count}}][unit]" class="form-control" value="{{ $product->sells_unit }}">
