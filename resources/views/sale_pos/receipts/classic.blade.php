...
<div class="row">
    ...
    <!--    <img src="{{ asset($receipt_details->logo) }}" class="img img-responsive center-block">-->
    <!--@endif-->
        <img style="width:15%;" src="{{ asset('/invoice_logos/ARK_Logo.jpg') }}" class="img img-responsive center-block">
    <!-- Header text -->
    ...
    <div class="col-xs-12 text-center">
        ...
        <p style="width: 100% !important" class="word-wrap">
            ...
            <span class="pull-right">
                ...
                @if ($receipt_details->sr_man_name != '')
                    <b>SR Name : {{ $receipt_details->sr_man_name }}</b> <br>
                @endif

                @if ($receipt_details->delivery_man_name != '')
                    <b>Delivery Man : {{ $receipt_details->delivery_man_name }}</b>
                @endif
            ...
            <tbody>
                ...
                @forelse($receipt_details->lines as $line)
                    <tr>
                        ...
                        <td> {{ $line['units'] }}</td>
                        <!--hey jack work here-->
                        <td>{{ $line['quantity'] }} {{ $line['sells_unit'] }} </td>
                        <td>{{ $line['quantity_returned'] }} pc </td>

                        {{-- <td>{{}} {{$line['units']}} </td> --}}
                        <td>{{ $line['unit_price_inc_tax'] }}</td>
                        <td>{{ $line['bonus_product'] }}</td>
                        <td>{{ $line['line_discount_org'] }}</td>
                        <td>{{ $line['line_total'] }}</td>

                        {{-- here will go the folowing free product row --}}
                    </tr>
...
<div class="row">
    ...
    <div class="col-md-6">
        <h4>Total Pices (PC) : {{ $receipt_details->total_pieces }}</h4>
        <h4>Total Carton (CR) : {{ $receipt_details->total_carton }}</h4>
        <h4>Bonus : {{ (int)$receipt_details->total_bonus }}</h4>
        <h4>Discount (-) : {{ $receipt_details->discount }}</h4>
        <h4>Total Payable Amount : {{ $receipt_details->total }} </h4>
...
<div  style="display:flex; margin-top: 20px; ">
    <div style="">
        <h5 class="">Checked and Received By</h5>
    </div>
    <div style="margin-left: 140px;">
        <h5 class="">Prepared BY</h5>
    </div>
</div>
