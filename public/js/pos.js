$(document).ready( function(){
	...
	$( "#search_product" ).autocomplete({
		source: function(request, response) {
    		...
    		$.getJSON("/pos-search-product", { price_group: price_group, location_id: $('input#location_id').val(), term: request.term }, response);
  			},
		minLength: 2,
	...
	// update bonus product 
	$('table#pos_table tbody').on('change', 'input.pos_quantity', function(){
		var tr = $(this).parents('tr');
		var unit = tr.find('input.product-unit').val();
		...
		var total_bonus = Math.floor((qty/sell_product) * bonus);
		console.log(total_bonus);
		__write_number(tr.find('input.discount_product_quantity'), total_bonus);		
	})
	
	// change Unit price 
    $('table#pos_table tbody').on('change', 'input.default_unit_price', function(){
		var tr = $(this).parents('tr');
		var unit = tr.find('input.product-unit').val();
		var unit_price_inc_tax = tr.find('input.default_unit_price').val();
		var child_value = __read_number(tr.find('input.child-value'));
		if (unit === 'cr') {
			unit_price_inc_tax = unit_price_inc_tax*child_value;
		}
	...
	// console.log(pos_form_validator);
...
function pos_each_row(row_obj){
	...
	var sell_price =__read_number(row_obj.find('input.pos_unit_price_inc_tax')) ;
	var line_total = quantity * sell_price;
...
function calculate_billing_details(price_total){
	...
	if( $('form#edit_pos_sell_form').length == 0 ){
		__write_number($('.payment-amount').first(), 0.00);
	}
...
function reset_pos_form(){
	...
	if($('form#edit_pos_sell_form').length > 0){
		setTimeout(function() {
			window.location = '/arkfoods/pos/create/';
		}, 4000);
		return true;
	}
