...
$(document).ready(function () {
  ...
  var transaction_table = $("#transaction_table").DataTable({
    ...
    fnFooterCallback: function (nRow, aaData, iStart, iEnd, aiDisplay) {
      ...
      for (var i = 0; i < aaData.length; i++) {
        ...
        else total += parseFloat(aaData[aiDisplay[i]][4]);
      }
  ...
  if ($("form#bussiness_edit_form").length > 0) {
    ...
    $("input#purchase_in_diff_currency").on("ifChecked", function (event) {
      $(
        "div#settings_purchase_currency_div, div#settings_currency_exchange_div"
      ).removeClass("hide");
    });
  ...
  $(".location_add_modal, .location_edit_modal").on(
    "shown.bs.modal",
    function (e) {
  ...
  var transfer_balance_transaction_table = $(
    "#transfer_balance_transaction_table"
  ).DataTable({
    processing: true,
    serverSide: true,
  ...
  var transfer_balance_personal_transaction_table = $(
    "#transfer_balance_personal_transaction_table"
  ).DataTable({
    processing: true,
    serverSide: true,
  ...
  $(document).on(
    "submit",
    "form#transfer_balance_personal_add_form",
    function (e) {
      e.preventDefault();
      var data = $(this).serialize();
  ...
  $(document).on(
    "click",
    "button.delete_transfer_balance_personal",
    function () {
      swal({
        title: LANG.sure,
        text: "Are you sure?",
  ...
  var receive_balance_transaction_table = $(
    "#receive_balance_transaction_table"
  ).DataTable({
    processing: true,
    serverSide: true,
  ...
  var receive_balance_personal_transaction_table = $(
    "#receive_balance_personal_transaction_table"
  ).DataTable({
    processing: true,
    serverSide: true,
  ...
  $(document).on(
    "submit",
    "form#receive_balance_personal_add_form",
    function (e) {
      e.preventDefault();
      var data = $(this).serialize();
      $.ajax({
  ...
  $(document).on(
    "click",
    "button.delete_receive_balance_personal",
    function () {
      swal({
        title: LANG.sure,
        text: "Are you sure?",
  ...
  $("#todays_profit_modal").on("shown.bs.modal", function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        $(".modal_opening_stock").html(
          __currency_trans_from_en(data.opening_stock, true)
        );
        $(".modal_closing_stock").html(
          __currency_trans_from_en(data.closing_stock, true)
        );
        $(".modal_total_sell").html(
          __currency_trans_from_en(data.total_sell, true)
        );
        $(".modal_total_purchase").html(
          __currency_trans_from_en(data.total_purchase, true)
        );
        $(".modal_total_expense").html(
          __currency_trans_from_en(data.total_expense, true)
        );
        $(".modal_net_profit").html(
          __currency_trans_from_en(data.net_profit, true)
        );
