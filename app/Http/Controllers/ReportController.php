...
class ReportController extends Controller
{
   ...
   public function sellUpdateTrackingProduct($id)
   {
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $products = $query->select(
                ...
                DB::raw("(SELECT SUM(IF(transactions.type='sell', IF(TSL.sells_unit = 'cr', (TSL.quantity * units.child_value)  + TSL.bonus_product_quantity, TSL.quantity ) - TSL.quantity_returned, -1 * TPL.quantity))
                    FROM transactions 
            ...
            return Datatables::of($products)
                ->editColumn('stock', function ($row) {
                    ...
                    if ($row->enable_stock) {
                        ...
                        return  '<span class="" data-orig-value="' . (float)$stock . '" data-unit="' . $row->unit . '" data-currency_symbol=false > ' . (float)$stock . '</span>' . ' ' . $row->unit ;
                    } else {
                        return 'N/A';
                    }
                })
                ...
                ->editColumn('total_sold', function ($row) {
                    ...
                    return '<span class="" data-currency_symbol=false data-orig-value="' . $total_sold . '" data-unit="' . $row->unit . '" >' . $total_sold . '</span> ' . $row->unit;
                })
                
                ->editColumn('mrp_price', function ($row) {
                    $amount = $row->unit_price * $row->stock;
                ...
                ->editColumn('initial_stock', function ($row) {
                        ...
                        return  '<span class="" data-orig-value="' . (float)$initial_stock . '" data-unit="' . $row->unit . '" data-currency_symbol=false > ' . (float)$initial_stock . '</span>' . ' ' . $row->unit ;
                ...
                ->editColumn('total_transfered', function ($row) {
                    ...
                    return '<span class="" data-currency_symbol=false data-orig-value="' . $total_transfered . '" data-unit="' . $row->unit . '" >' . $total_transfered . '</span> ' . $row->unit;
                })
                ->editColumn('total_adjusted', function ($row) {
                    ...
                    return '<span class="" data-currency_symbol=false  data-orig-value="' . $total_adjusted . '" data-unit="' . $row->unit . '" >' . $total_adjusted . '</span> ' . $row->unit;
                })
                ...
                ->removeColumn('unit')
                ->removeColumn('id')
                ->rawColumns(['sku','product','unit_price', 'total_transfered', 'total_sold',
                    'total_adjusted', 'stock','initial_stock', 'mrp_price'])
                ->make(true);
        }
    ...
    public function customerWiseSellReport(Request $request){
        $business_id = request()->session()->get('user.business_id');
        if (request()->ajax()) {
            $business_id = request()->session()->get('user.business_id');
            $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            if (!empty(request()->sr_id)) {
                $sr_id = request()->sr_id;
                $sells->where('transactions.sr_id', $sr_id);
            }
            ...
            return Datatables::of($sells)
                ...
                ->rawColumns(['final_total', 'total_paid', 'total_remaining', 'payment_status', 'invoice_no'])
                ->make(true);
        }
       
        $customers = Contact::customersDropdown($business_id, false);
        return view('report.customer_wise_sell_report', compact('customers'));
    }
    
    public function customerWiseDueSell(Request $request){
        ...
        $customers = Contact::customersDropdown($business_id, false);
        return view('report.customer_wise_due_sell_report', compact('customers'));
    }
    
    public function getDueSellReport() {
        ...
        $sells_representatives = SellsRepresentative::pluck('name','id')->toArray();
        return view('report.due_sell_report', compact('sells_representatives'));
    }
    ...
    public function productWiseSell(Request $request){
        ...
        if ($request->ajax()) {
            ...
            $query = DB::table('products as p')
                ...
                ->select(
                    ...
                    'p.sku',
                    'u.short_name as unit',
                    DB::raw('SUM(tsl.quantity * tsl.unit_price_inc_tax) as subtotal'),
                    DB::raw('SUM(tsl.quantity) as sell_qty')
                )
                ->groupBy('p.id');
            ...
            $sr_id = $request->get('sr_id', null);
            if (!empty($sr_id)) {
                $query->where('t.sr_id', $sr_id);
            }
        ...
        $brands = Brands::where('business_id', $business_id)
        ->pluck('name', 'id');
        return view('report.new_product_sell')
            ->with(compact('business_locations', 'sells_representatives','categories','brands'));
    ...
    public function getStockTrackReport(Request $request){
        ...
        $query = DB::table('stock_trackings as ST')
                ->join('products as p','p.id','=','ST.product_id')
                ->select('p.name','p.sku','ST.*');
            $location_id = $request->get('location_id', null);
            if (!empty($location_id)) {
                $query->where('ST.location_id', $location_id);
            }
            ...
            if (!empty($start_date) && !empty($end_date)) {
                $query->whereBetween(DB::raw('date(ST.created_at)'), [$start_date, $end_date]);
            }else{
                $query->wheredate('ST.created_at',\Carbon::now()->format('Y-m-d'));
            }
        return DataTables::of($query)
        ...
        $business_locations = BusinessLocation::forDropdown($business_id, true);
        return view('report.stock_track',compact('business_locations'));
    }
}
