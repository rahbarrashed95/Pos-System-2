        ...
        try {
            ...
            if ($request->hasFile('products_csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    //Custom fields
                    if (isset($value[4])) {
                        $product_array['product_custom_field1'] = trim($value[4]);
                    } else {
                   ...
                   $product_type = "single";
                    ...
                    //Add barcode type
                      $barcode_type = strtoupper(trim(!empty($value[6])?($value[6]):"C128"));
                    ...
                    //Add Tax
                    $tax_name = trim(!empty($value[11])?($value[11]):"");
                    $tax_amount = 0;
                    if (!empty($tax_name)) {
                    ...
                    //Add tax type
                    $tax_type = strtolower(trim(!empty($value[12])?($value[12]):"inclusive"));
...
$category_name ="";
                    ...
                    //Add SKU
                    $sku = trim(!empty($value[5])?($value[5]):"");
                    if (!empty($sku)) {
                    ...
                    //Add product expiry
                    $expiry_period = $this->productUtil->num_uf(trim(!empty($value[9])?($value[9]):""));
                    $expiry_period_type = strtolower(trim(!empty($value[10])?($value[10]):""));
                    ...
                    $enable_sr_no = trim(!empty($value[23])?($value[23]):"0");
                    ...
                    if ($product_array['type'] == 'single') {
                        ...
                        $profit_margin = trim(!empty($value[18])?($value[18]):"");
                        if (empty($profit_margin)) {
                            $profit_margin = $default_profit_percent;
                        } else {
                            $profit_margin = $this->productUtil->num_uf(trim(!empty($value[18])?($value[18]):""));
                        }
                        ...
                        $dpp_inc_tax =  $mrp - (($mrp * 14.25) / 100);
                        $dpp_exc_tax = trim(!empty($value[17])?($value[17]):"");
                      ...
                      $mrp = trim($value[3]) * 1;
                        ...
                        $dpp_inc_tax_string = $mrp - (($mrp * 14.25) / 100);
                        $dpp_exc_tax_string = trim(!empty($value[17])?($value[17]):"");
                       
                            $mrp = trim($value[3]) * 1;
                        
                        $selling_price_string = $mrp - (($mrp * 14) / 100);
                        $profit_margin_string = trim(!empty($value[18])?($value[18]):"");
                ...
                if (!empty($formated_data)) {
                    foreach ($formated_data as $index => $product_data) {
                        ...
                        //Rack, Row & Position.
                        $this->rackDetails(
                            !empty($imported_data[$index][25])?($imported_data[$index][25]):""   ,
                           !empty($imported_data[$index][26])?($imported_data[$index][26]):""   ,
                          !empty($imported_data[$index][27])?($imported_data[$index][27]):""   ,
                            $business_id,
                            $product->id,
                            $index+1
                        );
