...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id)
    {
        ...
        $transaction = Transaction::create([
            ...
            'delivery_man_id' => !empty($input['delivery_man']) ? $input['delivery_man'] : null,
            'sr_id' => !empty($input['sr_id']) ? $input['sr_id'] : null,
        ]);
     ...
     */
    public function createOrUpdateSellLines($transaction, $products,$contact_id, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [])
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
              ...
              //end code
                $line = [
                    ...
                    'sells_unit' => $product['unit'],
                    'sell_line_note' => !empty($product['sell_line_note']) ? $product['sell_line_note'] : '',
                    'bonus_product_quantity' =>!empty($product['discount_product_quantity']) ? $product['discount_product_quantity'] : 0
                ];
    ...
    public function editSellLine($product, $location_id, $status_before)
    {
        ...
        if ($status_before != 'draft') {
            ...
            $bonusDeferance = $sell_line->bonus_product_quantity - $product['discount_product_quantity'];
            $this->adjustQuantity($location_id, $product, $product['variation_id'], $difference , $product['unit']);
           if($bonusDeferance != 0){
                $this->adjustQuantity($location_id, $product, $product['variation_id'], $bonusDeferance , $product['unit']);
            }
        }
    ...
    public function deleteSellLines($transaction_line_ids, $location_id)
    {
        if (!empty($transaction_line_ids)) {
            ...
            foreach ($sell_lines as $line) {
                $this->adjustQuantity($location_id, $line, $line->variation_id, $line->quantity , $line->sells_unit);
            }
     ...
     * @return boolean
     */
    private function adjustQuantity($location_id, $product, $variation_id, $increment_qty, $unit)
    {   
        $product_id = $product['product_id'];
        $product = Product::join('units','units.id','=','products.unit_id')
        ->leftjoin('units as child_unit','child_unit.child_id','units.id')
        ->select('products.enable_stock','child_unit.child_value', 'units.*')
        ->find($product_id);
        if ($increment_qty != 0) {
            if ($product->enable_stock == 1) {
                if (!empty($unit) && $unit ==='pc' && $product->child_value !=null) {
                    $qty = $increment_qty;
                    VariationLocationDetails::where('variation_id', $variation_id)
                    ->where('product_id', $product_id)
                    ->where('location_id', $location_id)
                    ->increment('qty_available', $qty);
                }else if (!empty($unit) && $unit =='cr' && $product->child_value !=null){
                $qty =($increment_qty * $product->child_value);
    ...
    $transaction = Transaction::leftjoin('sells_representatives', 'transactions.sr_id', '=','sells_representatives.id' )
    ->leftjoin('delivery_men','transactions.delivery_man_id', '=', 'delivery_men.id' )
        ->select('transactions.*', 'sells_representatives.name as sr_name', 'delivery_men.name as delivery_man_name'  )
        ->find($transaction_id);
        // dd($transaction);

    //returnt details
    $sell_return_transaction = Transaction::where('type','sell_return')->where('return_parent_id', $transaction->id )->first();
    if($sell_return_transaction){
        $returnt_total_amount = $sell_return_transaction->final_total;
    }
    else{
        $returnt_total_amount =0;
    }
    ...
    $transaction_type = $transaction->type;
        ...
        if ($il->show_customer == 1) {
            ...
            if (!empty($output['customer_info']) && $receipt_printer_type != 'printer') {
                ...
                $output['customer_info'] .= '<br/> Tel: ' . $customer->mobile;
            }
        ...
        // delivery man and sr name 
        $output['delivery_man_name'] ='';
        $output['sr_man_name'] ='';
        if($transaction->sr_id != null){
            $output['sr_man_name'] = $transaction->sr_name;
        }

        if($transaction->sr_id != null){
            $output['delivery_man_name'] = $transaction->delivery_man_name;
        }
        $output['client_id'] = '';
        ...
        if ($transaction_type == 'sell') {
            ...
            $details = $this->_receiptDetailsSellLines($lines, $il, $is_product_expiry_enabled, $is_lot_number_enabled , $show_currency );
            $output['lines'] = $details['lines'];
            $output['taxes'] = [];

            foreach ($details['lines'] as $line) {
            if($line['sells_unit'] == 'cr'){
                $total_carton += (($line['quantity']) - ($line['quantity_returned']));
            }else if($line['sells_unit'] == 'pc'){
                $total_pieces += (($line['quantity']) - ($line['quantity_returned']));
            }
        ...
        // show total carton and pieces
        $output['total_carton'] = $total_carton;
        $output['total_pieces'] = $total_pieces;
        ...
        $total_bonus = 0;
        foreach ($details['lines'] as $line) {
            $total_line_discount += $line['line_discount'];
            if ($transaction_type == 'sell') {
            $total_bonus += $line['bonus_product'];
            }
        }
        ...
        $output['discount'] =  $this->num_f(($discount + $total_line_discount), $show_currency) ;
        //Format tax
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            $sells_total_paid = ($paid_amount == 0) ? 0 : $paid_amount;
            $sells_total_paid_label = $il->paid_label;
            $sells_total_due = ($due == 0) ? 0 : $due;
            $sells_total_due_label = $il->total_due_label;
            //Get payment details
            $output['payments'] = [];
        ...
        if ($sell_return_transaction && $sell_return_transaction->status == 'final') {
            $paid_amount = $this->getTotalPaid($sell_return_transaction->id);
            $due = $sell_return_transaction->final_total - $paid_amount;

            $return_total_paid = ($paid_amount == 0) ? 0 :$paid_amount;
            $return_total_paid_label = $il->paid_label;
            $return_total_due = ($due == 0) ? 0 : $due;
            $return_total_due_label= $il->total_due_label;    
        }

        if ($transaction_type == 'sell' && $transaction->status == 'final') {
        $output['total_paid'] = $this->num_f(($sells_total_paid -  $return_total_paid), $show_currency);
        $output['total_paid_label'] =  $sells_total_paid_label;
        $output['total_due'] = $this->num_f(($sells_total_due - $return_total_due) , $show_currency) ;
        $output['total_due_label'] =  $sells_total_due_label;
        }


        //Total
        $total =$transaction->final_total - $returnt_total_amount;
        
        // number in words
        // $numberFormatter = new NumberFormatter('en', NumberFormatter::SPELLOUT);
        // $words = $numberFormatter->format($total);
        // if (strpos($total, '.') !== false) {
        //     $parts = explode('.', $total);
        //     $integerPart = $parts[0];
        //     $decimalPart = $parts[1];
        //     $integerWords = $numberFormatter->format($integerPart);

        //     if ($decimalPart != 0) {
        //         $decimalWords = $numberFormatter->format($decimalPart);
        //         $decimalUnit = $decimalPart == 1 ? 'pence' : 'pence';
                
        //         $words = $integerWords . ' pound and ' . $decimalWords . ' ' . $decimalUnit;
        //     } else {
        //         $words = $integerWords . ' pound';
        //     }
        // }

        $output['in_words'] =  $words= null;
        ...
        if ($transaction_type == 'sell_return') {
            ...
            $output['total'] = $this->num_f($total , $show_currency);
        }
     ...
     * @return array
     */
    protected function _receiptDetailsSellLines($lines, $il, $is_product_expiry_enabled, $is_lot_number_enabled, $show_currency)
    {
        $output_lines = [];
        //$output_taxes = ['taxes' => []];
        foreach ($lines as $line) {
            ...
            if($line->sells_unit == 'cr'){
                $return_unit = $line->unit_price_inc_tax / $unit->child_value; 
            }else{
                $return_unit = $line->unit_price_inc_tax;
            }
            $line_array = [
                ...
                'quantity_returned' => $this->num_f($line->quantity_returned),
                'units' => !empty($unit->short_name) ? $unit->short_name : '',
                'child_value' => !empty($unit->short_name) ? $unit->child_value : '',
                ...
                'unit_price_inc_tax' => $this->num_f($line->unit_price_inc_tax, $show_currency),
                'unit_price_exc_tax' => $this->num_f($line->unit_price , $show_currency ),
                'price_exc_tax' => $line->quantity * $line->unit_price,
                'unit_price_before_discount' => $this->num_f($line->unit_price_before_discount, $show_currency),
                'bonus_product' => ($line->bonus_product_quantity),
                'sells_unit' => ($line->sells_unit),
                'line_total' => $this->num_f(( ($line->unit_price_inc_tax * $line->quantity) - ($line->quantity_returned * $return_unit ) ) , $show_currency),
            ];
            ...
            $line_array['line_discount_org'] = $line_array['line_discount'];
            if ($line->line_discount_type == 'percentage') {
                $line_array['line_discount_org'] .= ' (' . $this->num_f($line->line_discount_amount) . '%)';
                $line_array['line_discount'] = ($line->line_discount_amount/100) *$variation->sell_price_inc_tax;
            }
    ...
    protected function _receiptDetailsSellReturnLines($lines, $il, $is_product_expiry_enabled, $is_lot_number_enabled)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                //Field for 3rd column
                'unit_price_inc_tax' => $this->num_f($return_unit),
                'unit_price_exc_tax' => $this->num_f($return_unit),

                //Fields for 4th column
                'line_total' => $this->num_f($return_unit * $line->quantity_returned),
            ];
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            // if ( ($qty_selling == 0 || $qty_selling ==null)) {
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            ...
            if (!empty($sell_purchases)) {
                ...
                foreach ($sell_purchases as $row) {
                        ...
                        $data = PurchaseLine::where('id', $row['purchase_line_id'])
